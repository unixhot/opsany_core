...
class ControlRunScript(base_action.OpsAnyCoreRestAPI):
    def run_script(self, host_list, script_url, script_arg=None):
    ...
    def run(self, host_list, script_url, script_arg=""):
        requests_id = self.run_script(host_list, script_url, script_arg)
        if not requests_id:
...
if __name__ == '__main__':
    host_list, script_url = ["dev_node2"], "/uploads/"
    res = ControlRunScript().run(host_list, script_url, "")
    print("res", res)
